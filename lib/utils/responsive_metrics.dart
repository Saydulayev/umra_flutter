...
class ResponsiveMetrics {
  ...
  // ── Card radius tokens ──────────────────────────────────────────────────
  //
  // Раньше радиусы карточек были раскиданы локальными константами по
  // экранам (24 в home/dua_book, 20 в prayer_time, 24/28 в useful_info) —
  // на телефоне почти совпадали случайно, на планшете расходились без
  // единой логики. Ниже — два семейства, единый источник правды. Меняются
  // только планшетные значения (телефон не затронут): 24-семейство
  // дотягивается до уже принятого в проекте паттерна useful_info (28 на
  // планшете), 20-семейство — до паттерна arabicCardRadius (24 на планшете).
  //
  // Декоративные элементы фиксированного размера (иконки-плашки в
  // hajj_icon_widget, превью изображений в styled_image_widget) сюда
  // намеренно не включены — это другая категория UI (медиа/иконки, а не
  // прокручиваемые карточки контента), объединять их радиус с карточками
  // была бы отдельная, более смелая правка.

  /// Радиус основных карточек-контейнеров: сгруппированные списки шагов на
  /// главном экране, книга дуа, полезная информация.
  double get primaryCardRadius => isTablet ? 28 : 24;

  /// Радиус вторичных карточек среднего размера: время намаза, арабский
  /// текст, второстепенные инфо-карточки (перевод/транслитерация дуа).
  double get secondaryCardRadius => isTablet ? 24 : 20;

  /// Радиус арабской текстовой карточки. Тот же токен, что и
  /// [secondaryCardRadius] — арабские карточки относятся к тому же
  /// визуальному семейству, отдельного числа не требуется.
  double get arabicCardRadius => secondaryCardRadius;
